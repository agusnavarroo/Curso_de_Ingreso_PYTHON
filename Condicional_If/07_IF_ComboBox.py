...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        naturalizado = self.combobox_tipo.get()
        if edad_numero < 16 :
            alert(title="Alerta", message="Usted no esta autorizado a votar")
        else : 
            if edad_numero >= 16 and nativo == "NATIVO" :
                alert(title="Alerta", message="Usted esta autorizado a votar")
            else :
                if edad_numero >= 16 and edad_numero <= 17 and naturalizado == "NATURALIZADO" :
                    alert(title="Alerta", message="Usted no esta autorizado a votar")
                else : 
                    if edad_numero >= 18 and naturalizado == "NATURALIZADO":
                        alert(title="Alerta", message="Usted esta autorizado a votar")
