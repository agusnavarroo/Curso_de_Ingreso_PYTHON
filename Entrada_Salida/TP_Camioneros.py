...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        ...
        toneladas_numero = float(self.txt_toneladas.get())
        cantidad_camiones = toneladas_numero / 3.5
        mensaje = "Se necesitaran {0} camiones".format (cantidad_camiones)
        alert(title="Camiones", message=mensaje)

    def btn_tiempo_llegada_on_click(self):
        kilometros_texto = self.txt_kilometros.get()
        kilometros_numero = float(self.txt_kilometros.get())
        tiempo_necesario = kilometros_numero / 90
        mensaje = "Se tardaran {0} horas".format(tiempo_necesario)
        alert(title="Tiempo", message=mensaje)
